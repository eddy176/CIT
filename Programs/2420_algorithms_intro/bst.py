...
class BST:
    ...
    def traverse(self, callback, current):
        ...
        if dummy < current.mItem:
            ...
            if current.mLeft is None and current.mRight is None:
                ...
                while s.mLeft is not None:
                    s = s.mLeft
                self.deleteR(s.mItem, current.mRight)
        return current
...
def main():
    ...
    fretrieve = open('RetreiveNames.txt', 'r')
    for line in fretrieve:
        line = line.split()
        s = Students('', '', line[0], '', '')
        if not students.retrieve(s):
            print("Social Not Found")
