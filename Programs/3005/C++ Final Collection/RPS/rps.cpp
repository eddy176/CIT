...
int getRandomPlayerChoice(){///////////////
	int cChoice = (rand() % 4) + 1;
	return cChoice;
}

int getUserPlayerChoice(std::istream& input_stream, std::ostream& output_stream){
	std::string cInput;
	output_stream << "Choice? (rock, paper, scissors, well). " << std::endl;
	input_stream >> cInput;
	while (cInput != "rock" && cInput != "paper" && cInput !=  "scissors" && cInput != "well"){
	    output_stream << "Choice? (rock, paper, scissors, well). " << std::endl;
	    input_stream >> cInput;
	    if (cInput != "rock" && cInput != "paper" && cInput != "scissors"){
	        output_stream << "Please input proper choice. (rock, paper, scissors, well)" << std::endl;
	    }
	}
	if (cInput == "rock"){
