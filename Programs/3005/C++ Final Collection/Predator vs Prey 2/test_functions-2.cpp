...
TEST(simulation, SimulatesCleanup) {
	srand(time(NULL));
	Simulation sim(5, 5);
	std::vector< Critter* > dedCritters = sim.getCritters();
	size_t i;
	for (i = 0; i < dedCritters.size(); i++) {
		dedCritters[i]->kill();
	}
	sim.setCritters(dedCritters);
	sim.removeDead();
	EXPECT_EQ(true, sim.getCrittersSize() == 0);
}

TEST(simulation, simulationStep) {
	srand(time(NULL));
	Simulation sim(5, 5);
	std::vector<Critter*> simOb = sim.getCritters();
	sim.step();
	sim.step();
	sim.step();
	EXPECT_EQ(true, sim.getCrittersSize() != simOb.size());
}