...
PPM::PPM():
    mWidth(0), mHeight(0), mMaxColor(0), mPicture(){
    }
...
PPM& PPM::flipImage(PPM& ppm) {
    int height = ppm.getHeight();
    int width = ppm.getWidth();
    int i,j,k,new_value, value;
    for (i = 0; i < ppm.mHeight; i++){
        for (j = 0; j < ppm.mWidth; j++){
            for (k = 0; k < 3; k++){
                new_value = ppm.getChannel(i, j, k);
                value = static_cast<int>(new_value);
                this->setChannel(height-i, width-j, k, value);
            }
        }
    }
    return *this;
...
PPM& PPM::operator+=(const double& rhs){
    int i, j, k;
    int A, B;
    A = static_cast<int>(rhs);
    int height = this->getHeight();
    int width = this->getWidth();
    for (i = 0; i < height; i++){
        for (j = 0; j < width; j++){
            for (k = 0; k < 3; k++){
                B = this->getChannel(i, j, k);
                this->setChannel(i, j, k, B + A);
            }
        }
    }
    return *this;
