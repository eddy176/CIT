...
ColorTable::ColorTable( const size_t& num_color ):
    mColors(num_color){
        this->colorVector.resize(num_color);
}
    ...
    const Color& ColorTable::operator[]( const int& i ) const{
        if (i >= 0 and i < int(this->colorVector.size())){
            return this->colorVector[i];
        }
    ...
    void ColorTable::setRandomColor( const int& max_color_value, const size_t& position ){
        srand(time(NULL));
        int r;
        if (max_color_value <= 255 and max_color_value >= 0 and position < colorVector.size()){
            r = rand() % (max_color_value + 1);
            colorVector[position].setRed(r);
            colorVector[position].setGreen(r);
            colorVector[position].setBlue(r);
    ...
    void ColorTable::insertGradient( const Color& color1, const Color& color2, const size_t& position1, const size_t& position2 ){
        ...
        if(position1 < position2){
            this->colorVector[position1] = color1;
            this->colorVector[position2] = color2;
            ...
            for(i = 0; i < distance; i++){
                colorVector[position1 + i].setChannel(0, (red1 + i * changeR));
                colorVector[position1 + i].setChannel(1, (green1 + i * changeG));
                colorVector[position1 + i].setChannel(2, (blue1 + i * changeB));
            }
        }
    }
