...
int main(){
    ...
    std::cout << "please input what you want to do! (C, r, g, b, l, G, u, a, o, old, v, h, add, sub, or flip) ";
    std::cin >> in;
    std::cout << std::endl;

    while (in != "C" && in != "r" && in != "g" && in != "b" && in != "l" && in != "G" && in != "u"
           && in != "a" && in != "o" && in != "old" && in != "v" && in != "h" && in != "add" && in != "sub" && in != "flip"){
    ...
    else if (in == "a"){    // if in is a, brighten the channel
        for (i = 0; i < height; i++){
            for (j = 0; j < width; j++){
                A = my_pic.getChannel(i, j, 0);
                B = my_pic.getChannel(i, j, 1);
