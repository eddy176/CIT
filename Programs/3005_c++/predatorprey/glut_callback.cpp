...
void display(void)
{
  ...
  */
  glColor3d(0, .85, 0);
  for(crit = 0; 0 < critters.size(); crit++) {
    if(crit->getFoodChainLevel() == 10) {
      DrawRectangle(g_app_data->getWidth(), g_app_data->getHeight(), 250, 250); // brown and tan for lions???
    }
    else if(crit->getFoodChainLevel() == 5){
      DrawRectangle(w / 10, h / 10, 250, 250); //white and black for zebras???
    }
  }
  glutSwapBuffers();
}
