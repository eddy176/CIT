...
JuliaSet::JuliaSet():
    mWidth(400), mHeight(400), minX(-2), maxX(2), minY(-2), maxY(2), mA(0), mB(0), maxEscapeCount(255), minEscapeCount(0){
    }
	...
	int JuliaSet::getMinEscapeCount() const {
		return minEscapeCount;
	}
	...
	void JuliaSet::setMinEscapeCount(const int& count) {
		minEscapeCount = count;
	}
    ...
    // calls calculated plane co-ordinates, checks if x and y are out of range, if they are, set to 0
