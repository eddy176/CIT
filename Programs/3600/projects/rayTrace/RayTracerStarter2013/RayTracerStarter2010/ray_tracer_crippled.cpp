...
// These are the calculations we did in class.
void Scene::CreateRay(int x, int y, Vector3 &ray)// called from main loop in initialize my stuff(for all x's, for all y's)
{
	if (first_time)
	{
		CalculateRightAndUp();
	...
	double xratio = (double)x / screen_x;

	// Calculate where x,y on the screen maps to on the WorldWindow (film.)
	double yratio = (double)y / screen_y;

	Point3 fp = ww_bl + (ww_br - ww_bl) * xratio + (ww_tl - ww_bl) * yratio;// point3 is already done
...
{
	// t is the distance between eye and closest sphere
	// intersection_point = eye + ray * t;
	// distance = smallest positive, t1 or t2
	// intersection_normal = intersection_point - sphere_center;
	...
	////////////////////////////////
	////////////////////////////////
	double A, B, C, desc, Dx, Dy, Dz, Ex, Ey, Ez, radius, Rx, Ry, Rz, RxT, RyT, RzT, Sx, Sy, Sz, T1, T2;
	Point3 ERT;
	Ex = eye.p[0];
	Ey = eye.p[1];
	Ez = eye.p[2];
	...
	//radius = pow(Ex, 2) + pow(Rx * T, 2) + pow(Ey, 2) + pow(Ry * T, 2) + pow(Ez, 2) + pow(Rz * T, 2);


	// A = Rx^2 + Ry^2 + Rz^2
	A = pow(Rx, 2) + pow(Ry, 2) + pow(Rz, 2);
	
	if (A == 0.0) {  // this prevents dividing by 0 in the quadratic formula
		return false;
	}

	// B = 2ESxRx + 2ESyRy + 2ESzRz
	B = (2 * Dx * Rx) + (2 * Dy * Ry) + (2 * Dz * Rz);

	// C = ESx^2 + EXy^2 + ESz^2 - radius^2
	C = pow(Dx, 2) + pow(Dy, 2) + pow(Dz, 2) - pow(radius, 2);

	// descriminent = sqrt(B^2 - 4AC)
	desc = pow( (pow(B, 2)) - 4 * A * C, .5);///////////////////////////////////?

	if (desc < 0) {  // prevents crash, it doesn't hit the sphere
		return false;
	}
	B *= -1;
	T1 = ( B + desc ) / (2 * A);// quadratic eq
	T2 = ( B - desc ) / (2 * A);
...
{// colors anything it hit grey
	...
	{
		// EVERYTHING WILL GO RIGHT HERE for part 2 of this assignment
		// ambient contribution
		r = s[closest_hit_sphere].ambient[0];
		g = s[closest_hit_sphere].ambient[1];
		b = s[closest_hit_sphere].ambient[2];
		// Diffuse Contribution (per light):
		for (size_t l = 0; l < num_point_lights; l++) {
			...
			if (dot > 0) {
				...
				// Specular contribution in video
				// if you go over 1 for red, blue, or green set it to 1.
				// take vector to light(normalized) and vector to eye(normalized) added, we get a third vector(not normalized)- so normalize it,
				// compare the sum vector with the normal vector, if they are on top of eachother, then the angles are equal- meaning the 
				// dot2 = dot product of the sum and normal would equal 1.0
				// with specular, if it's not right onto 1.0, it shouldn't reflect, raise dot2 to exponent to see if it's too far away, 
				// if it is, don't show it
				Vector3 toEye = eye - closest_point;// light diminuation????
				toEye.Normalize();
				Vector3 sum = toLight + toEye;
				sum.Normalize();
				float dot2 = DotProduct(sum, closest_normal);
				float dot2E = pow(dot2, s->specular_exponent);
				if (dot2E > 0.0) {
...
void InitializeMyStuff()
{
	...
	GLfloat white_light[] = {.9,.9,.9,1};
	GLfloat low_light[] = {.4,.4,.4,1};
