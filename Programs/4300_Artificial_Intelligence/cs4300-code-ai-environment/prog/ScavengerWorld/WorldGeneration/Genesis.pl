...
my $percent_mud    = .20;      # percent of plates that are muddy
my $percent_ice    = .00;      # percent of plates that are ice   (The rest are rocky)
...
my $base_count     = 1;        # number of bases to create
# configure above here

use Getopt::Std;
my %Options;

getopts("?w:h:l:b:n:m:M:p:u:c:o:P:f:F:s:B:i:", \%Options);
...
-p percent_plain  = ${percent_plain};      # percent of plates that are plain
-u percent_mud    = ${percent_mud};      # percent of plates that are muddy
-i percent_ice    = ${percent_ice};      # percent of plates that are icey (The rest are rocky)
...
-B base_count     = 1;        # number of bases to create
";
  exit 0;
}
...
if(defined($Options{'p'})) { $percent_plain = $Options{'p'}; }
if(defined($Options{'u'})) { $percent_mud = $Options{'u'}; }
if(defined($Options{'i'})) { $percent_ice = $Options{'i'}; }
if($Options{'c'}) { $cliff_height = $Options{'c'}; }
if(defined($Options{'o'})) { $max_objects = $Options{'o'}; }
if(defined($Options{'P'})) { $object_chance_power = $Options{'P'}; }
if($Options{'f'}) { $object_file = $Options{'f'}; }
if($Options{'F'}) { $output_file = $Options{'F'}; }
if($Options{'s'}) { $seed = $Options{'s'}; }
if($Options{'B'}) { $base_count = $Options{'B'}; }
...
foreach my $count (1..$num_continents)
{
  ...
  if($radius > int($world_width/2)) { $radius = int($world_width/2); }
  if($radius > int($world_height/2)) { $radius = int($world_height/2); }
  ...
  elsif($iface < ($percent_plain + $percent_mud + $percent_ice))
    {
      $iface = 'ice';
    }
  else
...
&DisplayCells(\@world, $output_file, $base_count);
...
sub DisplayCells
{
  my ($world, $filename, $base_count) = @_;
...
';
  if($percent_ice > 0)
  {
      print $fh 'Interface: ice        0     10           1       0', "\n";
  }
  
  my @bases = ();
  my $i;
  foreach $i (0..$base_count-1) { $bases[$i] = 0; }
  ...
  foreach $crow (@{$world})
    {
      foreach $cell (@{$crow})
        {
	    my $tmp_size = scalar(@{$world}) * scalar(@{$crow});
	    my $chance = (3. / $tmp_size); #/
          if($cell)
            {
	      foreach $i (0..$base_count-1)
	      {
		  if(($bases[$i] == 0) ||
		     (rand() < $chance))
		  {
