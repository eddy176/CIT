#ifndef _UTTTBOARD_H_
#define _UTTTBOARD_H_
namespace ai
{
  namespace UltimateTicTacToe
  {
    ...
    public:
      MoveData( );
      MoveData( int bx, int by, int px, int py );
      void setData( int bx, int by, int px, int py );
    ...
    enum WinReasonEnum { WIN_COLUMN_0, WIN_COLUMN_1, WIN_COLUMN_2,
                         WIN_ROW_0, WIN_ROW_1, WIN_ROW_2,
                         WIN_DIAGONAL_0, WIN_DIAGONAL_1,
                         WIN_NONE };
    ...
    public:
      SubBoardData( );
      void Reset( );
      bool Winner( int player ) const;
      bool Complete( ) const;
      WinReasonEnum FindPlayerWin( int player ) const;
      
      /*
       * The actual sub board data.
       *   0 means empty.
    ...
    public:
       ...
       * Subboards
       */
      SubBoardData grid[3][3];
      int num_players; // number of players in this game (2)
      int player_turn; // whose turn 1-num_players
      int next_board_x, next_board_y; // coordinates of next board to be played -1,-1 for any
    };
    ...
    public:
      ...
      /*
       * Place player's mark at position specified by move_in
       ...
       */
      virtual bool Occupied(int bx, int by, int x, int y) const;

      /*
       * Returns true if the board is usable and has no mark at x,y
       */
      virtual bool Available(int bx, int by, int x, int y) const;
