...
namespace ai {
  	namespace Abalone {
		...
		Agent::Agent(ai::Agent::Socket * sock_in) : ai::Agent::Agent(sock_in) {
			player_number = -1;
			duration = 0;
		}
		...
		bool Agent::AddToMessageIfChanged(ai::Agent::Message &omsg, ai::Agent::Message &old_msg) {
			...
			ADD_TO_MSG_IF_CHANGED("OBJECT_AGENT_PLAYER_NUMBER " << m_object_id, player_number, omsg, old_msg);
			ADD_TO_MSG_IF_CHANGED("OBJECT_AGENT_DURATION " << m_object_id, duration, omsg, old_msg);
			return true;
		}
		...
		bool Agent::SetFromMessageIfExists(unsigned int id, ai::Agent::Message &imsg) {
			...
			// player_number, duration
			SET_FROM_MSG_IF_EXISTS("OBJECT_AGENT_PLAYER_NUMBER " << m_object_id, player_number, imsg);
			SET_FROM_MSG_IF_EXISTS("OBJECT_AGENT_DURATION " << m_object_id, duration, imsg);
			return true;
		}
