  ...
  namespace TicTacToe
  {
    ...
    bool Environment::GetUpdateMessage(ai::Agent::Message &omsg)
    {
      ...
      for(i = 0; i < 3; i++)
        {
          for(j = 0; j < 3; j++)
            {
              ADD_TO_MSG_IF_CHANGED("GRID " << i << " " << j, board.grid[i][j], omsg, mLastMessage);
            }
        }
      ...
      for(ii = 0; ii < last_moves.size(); ii++)
        {
          ADD_TO_MSG_IF_CHANGED("LAST_MOVES " << ii, last_moves[ii].pos_x << " " << last_moves[ii].pos_y, omsg, mLastMessage);
        }
    ...
    bool Environment::UpdateFromMessage(ai::Agent::Message &imsg)
    {
      ...
      for(i = 0; i < 3; i++)
        {
          for(j = 0; j < 3; j++)
            {
              SET_FROM_MSG_IF_EXISTS("GRID " << i << " " << j, board.grid[i][j], imsg);
            }
        }
      ...
      for(i = 1; i <= board.num_players; i++)
        {
          SET_FROM_MSG_IF_EXISTS("LAST_MOVES " << (i-1), game_board.last_moves[i-1].pos_x >> game_board.last_moves[i-1].pos_y, imsg);
        }
