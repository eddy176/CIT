  ...
  namespace TicTacToe
  {
    bool Action::TextDisplay(std::ostream & os) const
    {
      ...
      switch(GetCode())
        ...
        case MOVE:
          ...
          {
            char c_str[64];
            std::sprintf(c_str, "%2d,%2d",
                         move.pos_x, move.pos_y);
            str += c_str;
          }
          break;
        case QUIT:
    ...
    bool Action::ToMessage(ai::Agent::Message &omsg) const
    {
      ...
      switch(action_code)
        {
        case MOVE:
          ADD_TO_MSG("MOVE", move.pos_x << " " << move.pos_y, omsg);
          DBGP_MSG(LOG_STREAM(std::cerr) << "Move: " << move.pos_x << "," << move.pos_y << std::endl);
          break;
        case QUIT:
          // OK, no other data
          break;
    ...
    bool Action::FromMessage(const ai::Agent::Message &imsg)
    {
      ...
      switch(action_code)
        {
        case MOVE:
          SET_FROM_MSG("MOVE", move.pos_x >> move.pos_y, imsg);
          DBGP_MSG(LOG_STREAM(std::cerr) << "Move: " << move.pos_x << "," << move.pos_y << std::endl);
          break;
        case QUIT:
          // OK, no other data
