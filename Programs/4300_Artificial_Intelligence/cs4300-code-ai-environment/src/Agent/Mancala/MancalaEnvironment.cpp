...
namespace ai
{
  namespace Mancala
  {
    ...
    bool Environment::GetUpdateMessage(ai::Agent::Message &omsg)
    {
       ...
       */
      ADD_VECTOR_TO_MSG_IF_CHANGED("BOARD", game_board.board.board, omsg, mLastMessage);
      ...
      ADD_TO_MSG_IF_CHANGED("GAME_OVER", game_board.board.game_over, omsg, mLastMessage);
      ...
      for(ii = 0; ii < game_board.last_moves.size(); ii++)
        {
          ADD_VECTOR_TO_MSG_IF_CHANGED("LAST_MOVES_BINS " << ii,
                                       game_board.last_moves[ii].bins,
                                       omsg, mLastMessage);
          ADD_VECTOR_TO_MSG_IF_CHANGED("LAST_MOVES_COUNTS " << ii,
                                       game_board.last_moves[ii].counts,
                                       omsg, mLastMessage);
        }
    ...
    bool Environment::UpdateFromMessage(ai::Agent::Message &imsg)
    {
       ...
       * BoardData information
       */
      SET_VECTOR_FROM_MSG_IF_EXISTS("BOARD", game_board.board.board, imsg);
      ...
      SET_FROM_MSG_IF_EXISTS("GAME_OVER", game_board.board.game_over, imsg);
      ...
      for(i = 1; i <= game_board.board.num_players; i++)
        {
          SET_VECTOR_FROM_MSG_IF_EXISTS("LAST_MOVES_BINS " << (i-1),
                                        game_board.last_moves[i-1].bins, imsg);
          SET_VECTOR_FROM_MSG_IF_EXISTS("LAST_MOVES_COUNTS " << (i-1),
                                        game_board.last_moves[i-1].counts, imsg);
        }
