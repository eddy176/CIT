...
namespace ai
{
  namespace Mancala
  {
    ...
    void EnvironmentDriver::Run()
    {
      ...
      if(tourney)
        runs *= 2;
      DBGP_L(LOG_STREAM(std::cerr) << "tourney: " << tourney
             << "  runs: " << runs
             << std::endl);
      for(int r = 0; r < runs; r++)
        {
          DBGP_L(LOG_STREAM(std::cerr) << "run start " << r << std::endl);
          ...
          int score  = final_board.Score(1) - final_board.Score(2);
          ...
          delete env;
          DBGP_L(LOG_STREAM(std::cerr) << "run done " << r << std::endl);
        }
    }
  }
}
