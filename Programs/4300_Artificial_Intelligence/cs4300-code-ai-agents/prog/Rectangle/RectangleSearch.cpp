...
namespace ai {
  namespace rectangle {
    ...
    static int search_init( AppData& data, ai::Search::Algorithm **algorithm ) {
      // initial state
      ai::rectangle::State *initial_state = new ai::rectangle::State( data.model );
      // problem
      ai::rectangle::Problem *problem = new ai::rectangle::Problem( initial_state );
      ...
      for( i = 1; i <= max_iterations && !solved; i++ ) {
        ...
        if( algorithm->Search( ) ) {
          ...
          for( it = solution->begin( ); it != solution->end( ); it++ ) {
            if( (*it)->GetAction( ) ) {
              ai::rectangle::Action *action = dynamic_cast< ai::rectangle::Action * >( (*it)->GetAction( ) );
              if( !first ) {
                moves << " ";
              } else {
                first = false;
              ...
              if( data.config.applySolution( ) ) {
                if( !data.model.ApplyMove( action->getMove( ) ) ) {
                  std::stringstream ss;
