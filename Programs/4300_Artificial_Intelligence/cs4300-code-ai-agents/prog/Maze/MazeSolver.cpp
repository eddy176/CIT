      ...
      for(it = solution->begin(); it != solution->end(); it++)
        {
          ...
          if((*it)->GetState())
            {
              const cgl::Maze::State * const state =
                dynamic_cast<const cgl::Maze::State * const>((*it)->GetState());
...
int main(int argc, char **argv)
{
  ...
  cgl::Maze::Model *the_model = new cgl::Maze::Model(maze_file);
  cgl::Maze::State *the_initial_state = new cgl::Maze::State();
  the_initial_state->SetX(the_model->GetStartX());
  the_initial_state->SetY(the_model->GetStartY());
  
  cgl::Maze::State *the_goal_state = new cgl::Maze::State();
  the_goal_state->SetX(the_model->GetGoalX());
  the_goal_state->SetY(the_model->GetGoalY());
  
  BuildSearch<cgl::Maze::State,
              cgl::Maze::Problem,
              cgl::Maze::Model>(search_type, frontier_type, max_depth, ids_max,
